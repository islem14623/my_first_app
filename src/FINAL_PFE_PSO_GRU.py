"""
PSO + GRU Pipeline - Final PFE Version
Complete pipeline with NO data leakage
Uses pre-computed PSO feature selection results from CNN run
...
# Import load_data from PSO module (feature selection skipped - using saved results)
...
def main():
    ...
    print("="*90)
    print("FINAL PFE PIPELINE: PSO FEATURE SELECTION + GRU")
    print("="*90)
    ...
    selected_indices = joblib.load("models/pso_selected_features.pkl")
    ...
    # ========================================
    # STEP 5: Reshape for GRU (add channel dimension)
    # ========================================
    ...
    plot_training_history(history, "pso_gru_training_history.png")
    ...
    print(f"Features Used (loaded from PSO CNN run): {len(selected_indices)} / {len(feature_names)}")
    ...
    model.save("final_pso_gru_model.keras")
    joblib.dump(scaler, "pso_gru_scaler.pkl")

    final_metrics = {
        "algorithm": "PSO + GRU",
        "features_loaded_from": "pso_selected_features.pkl",
    ...
    with open("pso_gru_final_metrics.json", "w") as f:
        json.dump(final_metrics, f, indent=2)

    print("✓ Model saved as 'final_pso_gru_model.keras'")
    print("✓ Scaler saved as 'pso_gru_scaler.pkl'")
    print("✓ Metrics saved as 'pso_gru_final_metrics.json'")
    print("✓ Training history plot saved as 'pso_gru_training_history.png'")
