"""
PSO + LSTM Pipeline - Final PFE Version
...
def build_lstm_model(n_features, learning_rate=0.001):
...
def main():
    ...
    print("="*90)
    print("FINAL PFE PIPELINE: PSO FEATURE SELECTION + LSTM")
    print("="*90)
    ...
    # ========================================
    # STEP 5: Reshape for LSTM (add channel dimension)
    # ========================================
    print("\n[STEP 5] Preparing data for LSTM...")

    X_train_lstm = X_train_selected.reshape(X_train_selected.shape[0], X_train_selected.shape[1], 1)
    X_val_lstm = X_val_selected.reshape(X_val_selected.shape[0], X_val_selected.shape[1], 1)
    ...
    model = build_lstm_model(n_features=len(selected_indices))
    ...
    plot_training_history(history, "pso_lstm_training_history.png")
    ...
    model.save("final_pso_lstm_model.keras")
    joblib.dump(scaler, "pso_lstm_scaler.pkl")
    joblib.dump(selected_indices, "pso_lstm_selected_features.pkl")

    # Save feature names for reference
    selected_feature_names = [feature_names[i] for i in selected_indices]
    with open("pso_lstm_selected_feature_names.txt", "w") as f:
    ...
    with open("pso_lstm_final_metrics.json", "w") as f:
        json.dump(final_metrics, f, indent=2)

    print("✓ Model saved as 'final_pso_lstm_model.keras'")
    print("✓ Scaler saved as 'pso_lstm_scaler.pkl'")
    print("✓ Selected features saved as 'pso_lstm_selected_features.pkl'")
    print("✓ Feature names saved as 'pso_lstm_selected_feature_names.txt'")
    print("✓ Metrics saved as 'pso_lstm_final_metrics.json'")
    print("✓ Training history plot saved as 'pso_lstm_training_history.png'")
