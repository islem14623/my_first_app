"""
GA + GRU Pipeline - Final PFE Version
Complete pipeline with NO data leakage
Uses pre-computed GA feature selection results from CNN run
...
# Import load_data from GA module (feature selection skipped - using saved results)
from ga_feature_selection import load_data
...
def main():
    ...
    print("="*90)
    print("FINAL PFE PIPELINE: GENETIC ALGORITHM + GRU")
    print("="*90)
    ...
    selected_indices = joblib.load("models/ga_selected_features.pkl")
    ...
    plot_training_history(history, "ga_gru_training_history.png")
    ...
    print(f"Features Used (loaded from GA CNN run): {len(selected_indices)} / {len(feature_names)}")
    ...
    model.save("final_ga_gru_model.keras")
    joblib.dump(scaler, "ga_gru_scaler.pkl")

    final_metrics = {
        "algorithm": "GA + GRU",
        "features_loaded_from": "ga_selected_features.pkl",
    ...
    with open("ga_gru_final_metrics.json", "w") as f:
        json.dump(final_metrics, f, indent=2)

    print("✓ Model saved as 'final_ga_gru_model.keras'")
    print("✓ Scaler saved as 'ga_gru_scaler.pkl'")
    print("✓ Metrics saved as 'ga_gru_final_metrics.json'")
    print("✓ Training history plot saved as 'ga_gru_training_history.png'")
